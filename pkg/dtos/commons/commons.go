...
import (
	...
	"github.com/faridEmilio/api_go_viajate/internal/logs"
	"github.com/faridEmilio/api_go_viajate/pkg/dtos/commonsdtos"
...
type Commons interface {
	...
	*/
	...
	/*
		Validaciones para la subida de imagenes
	*/
	// Validación Completa para Fotos de Perfil, hace todas las validaciones anteriores y devuelve fileExtension
	// NOTE: Aplica solucion para multiples lectura
	IsProfilePhotoValid(fileHeader *multipart.FileHeader) (fileData []byte, fileExt string, contentType string, erro error)
	ValidateMimeType(fileData []byte) (contentType string, erro error)
	ValidateImageDimensionsFromBuffer(fileData []byte) error
	ValidateAndExtractImageExtension(fileHeader *multipart.FileHeader) (string, error)
}
...
func (c *commons) ValidateAndExtractImageExtension(fileHeader *multipart.FileHeader) (string, error) {
...
func (c *commons) IsProfilePhotoValid(fileHeader *multipart.FileHeader) (fileData []byte, fileExt string, contentType string, erro error) {
	...
	contentType, err = c.ValidateMimeType(fileData)
	if err != nil {
	...
	fileExt, err = c.ValidateAndExtractImageExtension(fileHeader)
	if err != nil {
	...
	if err = c.ValidateImageDimensionsFromBuffer(fileData); err != nil {
...
func (c *commons) ValidateMimeType(fileData []byte) (contentType string, erro error) {
	if len(fileData) < 512 {
...
func (c *commons) ValidateImageDimensionsFromBuffer(fileData []byte) error {
